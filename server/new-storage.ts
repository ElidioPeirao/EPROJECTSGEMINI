...
import { eq, and, lt, gte, or, desc, asc, isNull } from "drizzle-orm";
...
export interface IStorage {
  ...
  // Tool rating methods
  rateToolByUser(toolId: number, userId: number, rating: number, comment?: string): Promise<boolean>;
  getToolRatings(toolId: number): Promise<any[]>;
  getUserRatingForTool(toolId: number, userId: number): Promise<any | null>;
  ...
  // Check and remove expired promo upgrades
  checkAndRemoveExpiredPromoUpgrades(): Promise<void>;
...
export class DatabaseStorage implements IStorage {
  ...
  constructor() {
    ...
    // Execute initial check for expired users
    this.checkAndRemoveExpiredPromoUpgrades();
  ...
  private scheduleExpiredPurchasesCheck() {
    ...
    const runExpirationCheck = async () => {
      try {
        const now = new Date();
        console.log(`[${now.toISOString()}] Starting expiration check...`);
        
        // Desativar compras expiradas
        const expiredPurchases = await db.update(schema.coursePurchases)
          .set({ active: false })
          .where(and(
        ...
        // Verificar e remover upgrades de códigos promocionais expirados
        await this.checkAndRemoveExpiredPromoUpgrades();
        
        console.log(`[${now.toISOString()}] Expiration check completed. ${expiredPurchases.length} course purchases expired.`);
      } catch (error) {
    ...
    setInterval(runExpirationCheck, 30 * 60 * 1000); // 30 minutos
    console.log('Scheduled expiration checks every 30 minutes');
  }
  
  async checkAndRemoveExpiredPromoUpgrades() {
    try {
      const now = new Date();
      console.log(`[${now.toISOString()}] Checking for expired user plans...`);
      
      // Buscar e processar usuários com planos expirados usando SQL direto
      const result = await pool.query(`
        UPDATE users 
        SET role = 'E-BASIC', role_expiry_date = NULL 
        WHERE role_expiry_date < NOW() 
        AND role IN ('E-TOOL', 'E-MASTER')
        RETURNING id, username, role, stripe_subscription_id
      `);
      
      const processedUsers = result.rows;
      console.log(`Found and processed ${processedUsers.length} users with expired upgrades`);
      
      // Criar notificações para os usuários processados
      for (const user of processedUsers) {
        try {
          await this.createNotification({
            title: "Plano Expirado",
            message: user.stripe_subscription_id 
              ? `Sua assinatura expirou e sua conta foi revertida para E-BASIC. Para continuar usando as funcionalidades avançadas, renove sua assinatura.`
              : `Seu upgrade expirou e sua conta foi revertida para E-BASIC. Para continuar usando as funcionalidades avançadas, adquira um novo plano ou use um código promocional.`,
            type: "warning",
            targetRole: "E-BASIC",
            userId: user.id
          });
          
          console.log(`Reverted user ${user.username} (ID: ${user.id}) to E-BASIC due to expiration`);
        } catch (notificationError) {
          console.error(`Error creating notification for user ${user.username}:`, notificationError);
        }
      }
      
      console.log(`[${now.toISOString()}] Expired plans check completed. Processed ${processedUsers.length} users.`);
    } catch (error) {
      console.error('Error checking and removing expired upgrades:', error);
    }
  }
  ...
  async getUser(id: number): Promise<schema.User | undefined> {
    ...
    // Verificar se o upgrade do usuário expirou em tempo real
    if (result && result.roleExpiryDate && result.roleExpiryDate < new Date() && 
        (result.role === 'E-TOOL' || result.role === 'E-MASTER')) {
      
      // Reverter para E-BASIC imediatamente
      ...
      console.log(`Real-time: Reverted user ${result.username} (ID: ${id}) from ${result.role} to E-BASIC due to expiration`);
      
      // Criar notificação sobre a expiração
      await this.createNotification({
        title: "Upgrade Expirado",
        message: `Seu upgrade para ${result.role} expirou e sua conta foi revertida para E-BASIC. Para continuar usando as funcionalidades avançadas, adquira um novo plano ou use um código promocional.`,
        type: "warning",
        targetRole: "E-BASIC",
        userId: id
      });
  ...
  async extendProStatus(userId: number, days: number): Promise<schema.User | undefined> {
    ...
    // Upgrade user role while maintaining their highest level
    // E-BASIC users get upgraded to E-TOOL
    // Others maintain their level but get extended expirations
  ...
  async createCourse(course: schema.InsertCourse): Promise<schema.Course> {
    // Garantir que o price seja string se fornecido
    const courseData = {
      ...course,
      price: course.price !== undefined && course.price !== null ? course.price.toString() : course.price
    };
  ...
  // Tool rating methods
  async rateToolByUser(toolId: number, userId: number, rating: number, comment?: string): Promise<boolean> {
    try {
      // Check if user already rated this tool
      const existingRating = await db.query.toolRatings.findFirst({
        where: and(
          eq(schema.toolRatings.toolId, toolId),
          eq(schema.toolRatings.userId, userId)
        )
      });

      if (existingRating) {
        // Update existing rating
        await db.update(schema.toolRatings)
          .set({ 
            rating, 
            comment: comment || null,
            updatedAt: new Date()
          })
          .where(eq(schema.toolRatings.id, existingRating.id));
      } else {
        // Create new rating
        await db.insert(schema.toolRatings).values({
          toolId,
          userId,
          rating,
          comment: comment || null
      ...
      const ratings = await db.query.toolRatings.findMany({
        where: eq(schema.toolRatings.toolId, toolId)
      });
  ...
  async getToolRatings(toolId: number): Promise<any[]> {
    return await db.query.toolRatings.findMany({
      where: eq(schema.toolRatings.toolId, toolId),
      with: {
        user: {
          columns: {
      ...
      orderBy: desc(schema.toolRatings.createdAt)
    });
  }

  async getUserRatingForTool(toolId: number, userId: number): Promise<any | null> {
    const rating = await db.query.toolRatings.findFirst({
      where: and(
        eq(schema.toolRatings.toolId, toolId),
        eq(schema.toolRatings.userId, userId)
      )
